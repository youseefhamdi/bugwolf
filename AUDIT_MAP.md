...
|---|---|---|
| `tools/runtime/mission_runner.py` | 3,061 | **Mission runner — the orchestrator brain.** Domain lanes as probe swarms: BOLA (direct/enumeration/scope/role/mass-assignment/hidden), header-trust, WAF bypass (X-Original-URL, path obfuscation, encoding, parser differential, case rotation, payload splitting), A/B/C auth matrix, FIN business-logic matrix (incl. a hand-rolled SHA-256 length-extension attack), contract/cloud/LLM matrices; verify lane replays winning techniques; lead ladder R1–T4. **v1.18: every dispatch carries its model slice** — `_build_research_context` attaches the member's bug-class Target-Model slice (U-stage payload + ranked hypotheses + dispro plans) via `understanding/dispatch.py`; parked classes are skipped at staffing with a recorded fact, no model ⇒ prompts unchanged. **v1.19: predicted-chain priority** — `_order_families_by_predictions` reorders the web-lane swarm predicted-class-first (stable, logged as `predicted_chain_priority`); no predictions ⇒ declared order. `http_probe`@93 (scope-gated), `replay_bola_technique`@267, `replay_bypass_technique`@665, `replay_auth_technique`@936, `_length_extend`@1411, `replay_fin_technique`@1523, `MissionRunner`@2242, `main`@2860 |
...
| `tools/runtime/session_context.py` | ~340 | **v1.13: session context store** (Phase 2.2) — per-credential model: live JWT header+claim decode, role inference with source (jwt→response→operator), object-ID inventory (bounded extractor), endpoint facts, identity matrix; structural redaction on every export (raw tokens memory-only / opt-in disk), claim-shape redaction; `to_model_dict()` = U4 artifact. `SessionContextStore`@~170 (`from_matrix`@~190, `observe_response`@~230, `identity_matrix`@~262, `save`/`load`@~300) |
| `tools/runtime/authed_crawl.py` | ~310 | **v1.13: authenticated per-credential crawl** (Phase 2.3) — same URL space per identity (anon/A/B/C) through the replay engine (scope+governor inherited; BackendRefused → status-0 fact); HTML fact harvest via stdlib parser (title/links/form schemas); `CrawlReport.differential_paths()` = the authz candidate list (facts); artifacts `crawl/access_matrix.json` + `pages.jsonl`; feeds SessionContextStore as a side effect. `AuthedCrawler`@~180 (`crawl`@~232, `persist`@~298) |
| `tools/runtime/understanding/__init__.py` | ~50 | **v1.17: Understanding Layer package** (Part VIII) — public API: `UnderstandingPipeline`, `ModelStore`, `Assumption`, `UArtifact` |
| `tools/runtime/understanding/base.py` | ~250 | **v1.17: primitives** — `Assumption` (statement/origin observed-inferred-documented/confidence/dispro plan/challenge), `UArtifact` (hash-chained to inputs), `ModelStore` (`state/targets/<slug>/model/`: per-stage files, U8 = JSONL seed list + meta sidecar, tamper-detecting `needs_recompute`, `chain()`) |
| `tools/runtime/understanding/stages.py` | ~800 | **v1.17: the nine stage engines** — U1 money/trust/entity extraction + model-type classification; U2 business-criticality ranking (money ×6, differentials ×8, OpenAPI); U3 workflows (purchase/auth/redemption/recovery/verification/onboarding/funds-out) + state machines; U4 roles/JWT algs/authz boundaries; U5 object-ID formats + client-controlled fields; U6 trust/header families; U7 capability map (role,object,verb,impact) ranked dollars→privilege→ATO/PII; U8 ledger ranked by fragility (1−confidence)×stage weight; U9 COVERAGE GATE (`COVERAGE_CLASSES`: 10 classes, hunts only with model support, else parked with reason) + `render_brief` |
| `tools/runtime/understanding/pipeline.py` | ~230 | **v1.17: strict sequential runner** — per-stage prerequisite guard (`StagePrerequisiteError`), fail-closed hollow-run refusal, incremental recompute via input hashes (unchanged ⇒ cached; changed ⇒ affected stages only; tampered ⇒ recomputed), `PipelineResult` (hunts/parked/ledger/model_hash/brief path) |
| `tools/runtime/understanding/__main__.py` | ~190 | **v1.17: CLI** (`python3 -m tools.runtime.understanding`) — fetches U1 pages + `/openapi.json` via the replay engine (scope-gated), loads mission crawl/session artifacts, prints coverage + brief; `--refresh` forces full rebuild |
| `tools/runtime/replay/hpack.py` | ~330 | **v1.20: HPACK codec (RFC 7541)** — RFC-correct 61-entry static table (position 0 = illegal-index placeholder), dynamic table with 32-byte entry overhead + eviction + **applied** size updates, no-Huffman doctrine (Huffman-coded strings rejected by name), non-conformant `raw=True` mode emitting the forbidden 0x40 byte pattern WITHOUT table insertion (stateful peer decoders diverge — the HPACK-layer smuggling primitive), `raw_header_block()` verbatim length-prefixed pairs preserving duplicate headers. `encode_headers`@~219, `decode_headers`@~269 |
| `tools/runtime/replay/h2.py` | ~570 | **v1.20: HTTP/2 pseudo-layer (master plan 1.1b)** — client: `client_preface`/`build_headers_frame`/`build_data_frame`/`build_h2_request`/`split_frames` (preface-skip mode) through the governed `send_raw`; server: `H2Frontend`, a deliberately-flawed H2→H1.1 gateway with `forward_transfer_encoding` as the desync switch. **Safe mode owns framing** (TE stripped + C-L synthesized — the control test caught the first draft leaking the client's TE verbatim, re-poisoning the pool); per-connection backend read buffers make recv() over-delivery visible (a smuggled response hides exactly there); HTTP/1.1 passthrough on the same port (read1() not read() — BufferedReader.read(n) deadlocks short requests); keep-alive pool hygiene. `encode_frame`@~60, `parse_frame_header`@~95, `Handler.handle`@~196, `_dispatch`@~267, `_backend_roundtrip`@~338, `_read_http11`@~380, `_serve_http11`@~426 |
...
| `hooks/bugwolf_hooks.py` | ~420 | **v1.22: hooks 3.2 + 3.3 + cockpit 3.4 (Phase 3 completion)** — stdlib-only shim (JSON in → JSON out → exit 0; a failing hook is logged and swallowed so it can never stall the harness). **3.2 UserPromptSubmit**: mission context via `hookSpecificOutput.additionalContext` (target/boundary from scope contract, open-lead count) + TARGET MODEL STALE warning past `BUGWOLF_MODEL_MAX_AGE_H` (default 24h) / no-model nudge when bound. **3.3 PostToolUse**: HTTP-ish outputs → hash-chained `state/orchestrator/<m>/evidence.jsonl` (`prev_head → entry_hash`, head in `evidence_head`), `replay_key` = SHA-256(mission ⊦ target ⊦ method ⊦ path ⊦ head); bugwolf replay reports captured natively (method/path parsed from `sent_bytes` request line); conservative bounded extraction. **3.4 SessionStart cockpit**: scope + preflight digest + sandbox kill-switch/grants + leads by status + mode + target-model freshness. `user_prompt_submit`@~150, `post_tool_use`@~260, `session_start`@~380, `model_freshness`@~110 |
| `tools/runtime/capture_addon.py` | ~300 | **v1.21: mitmproxy capture addon (master plan 2.4, capture half)** — SELF-CONTAINED (zero bugwolf imports; runs in mitmproxy's interpreter); one JSONL line per exchange with byte-exact downgraded HTTP/1.1 wire text (original header case/order, latin-1 round-trip); framing/hop-by-hop headers + h2 pseudo-headers WITHHELD from emitted wire text while their upstream presence is recorded as `framing_notes` facts (TE on H2 = the H2.CL pre-condition; TE+C-L = the RFC 7230 §3.3.3 ambiguity candidate); suffix allow-list (`+.host` matches apex + subdomains, never `evilhost`); request-only records on connection errors. `CaptureAddon`@~150 (`response`@~210, `error`@~230, `_request_wire`@~130), `_CaptureWriter`@~60 |
| `tools/runtime/capture_replay.py` | ~340 | **v1.21: capture→replay import path (2.4, replay half)** — fail-closed loader (every line validates fully or is skipped WITH reason + line number; schema `bugwolf-capture/v1` in lockstep with the addon — a first-draft name collision would have silently rejected every real capture); **a capture file never widens scope** (gate bound to mission target BEFORE any send; out-of-scope records skipped as counted facts; port-tolerant suffix matching; explicitly-bound gate refuses rebinding); replay through the governed engine (`replay_raw`); drift facts (captured response = baseline, fresh send = experiment; status/body movement = cache variance / session carry-over leads, never verdicts); artifacts `mission/captures/capture_replays.jsonl` + `capture_report.json`; capture file never modified; CLI `--target/--rate/--budget/--marker`. `load_captures`@~150, `replay_captures`@~220, `_host_in_scope`@~200, `main`@~330 |
...
| `tools/runtime/scope.py` | 358 | **Deny-by-default operator scope gate.** Target host + explicit `--scope` entries allowed; `--exclude` carve-outs ALWAYS beat wildcards; fail-closed `ScopeViolation`; process-global, idempotent re-bind, refuses target mixing. v1.14 adds contract persistence for the PreToolUse hook: `write_scope_contract`/`clear_scope_contract` (state/scope_contract.json — the hook's authority exists exactly while the mission runs) + PEP 562 `GATE` module attribute. `ScopeGate`@56 (`check`@124), `bind_target`@196, `check_url`@209, `load_scope_file`@229, `write_scope_contract`@~250 |
| `tools/runtime/understanding/dispatch.py` | ~230 | **v1.18: model-slice dispatch (§8.3)** — `CLASS_STAGES` maps each of the 10 coverage classes to its feeding stages (idor→U4+U5, broken-business-logic→U1+U3, jwt-confusion→U4, ssrf/open-redirect→U6, …); `model_slice()` loads the store and extracts the per-class payload (roles, object-ID inventory, workflows, money paths, header families, parked reasons); `render_prompt_block()` renders the operator-visible intel block with ranked hypotheses + dispro plans; `dispatch_gate()` enforces the coverage gate at dispatch — PARKED class ⇒ skip with recorded fact, no model ⇒ no model ⇒ no-op pass-through |
| `tools/runtime/understanding/chain_predict.py` | ~300 | **v1.19: U7×U8 chain prediction (§8.3 completion)** — `ChainPredictor.predict()` pairs granted capabilities (U7: role × object × impact × path) with open assumptions about that object (U8 seed list; 0.05–0.9 confidence window); priority = impact (dollars→privilege→ATO→business) + (2−fragility) + terminal bonus; stage→class map (U3→business-logic, U4/U7→authz-bypass, U5→mass-assignment); terminal chaining via `deep_chain.EDGES` BFS; `registry_bug_class()` maps to registry ownership vocabulary; persistence `predicted-chains.json` (schema-checked, cap 20); `_brief_lines()` renders the brief's "Predicted chains — dispatch first" section |
| `tools/runtime/team.py` | 1,213 | **v1.4: Multi-Agent Team Engine** — waves recon→hunt(parallel specialists)→verify→report; ThreadPoolExecutor bounded by budget; append-only `team/runs.jsonl` ledger; atomic `team/state.json` checkpoints (write+fsync+rename); `TeamEngine`@~215 (`plan`@~330, `run`/`resume`@~430, `_recover_stale`@~410 heartbeat>15min fail-closed, `stop`@~570); member dispatch carries prompt+digest+scope+sandbox flags; no worker bound ⇒ BLOCKED evidence (first-class terminal), never fabricated; typed `TeamMessage` handoffs persisted to `messages.jsonl`. **v1.7: finding-driven recomposition** — hunt members may recommend unstaffed bug classes (`recommended_bug_classes` result field or `kind: agent_recommendation` messages); `_recommendations_from_results` + `_add_specialist`@~mid grow the roster mid-mission (registry-deterministic selection, budget-capped, deduped, workflow-safe); grown rosters re-enter the hunt wave before verify; every add/skip recorded in `state["recompositions"]` + runs ledger (`recomposed` events) + `TEAM_RECOMPOSED` signals; `--no-recompose` pins the roster; preference persists in `state.json` (`recompose`) and survives resume. **v1.8: architecture/ops hardening** — single shared wave driver `_drive_waves` (`run()`/`resume()` no longer duplicate loop logic), `recompose_waves=(recon,hunt)` (recon findings staff hunt specialists pre-hunt), `max_recompose_rounds=3` re-entry cap (`recompose_capped`/`recompose_rounds` recorded), idempotent ledger via `_recomposed_seen` (rehydrated on load), `preflight()` + CLI `--preflight` readiness report (no execution, no state writes). **v1.9.2: `_recon_depth_report()`** — shared `status()`/`preflight()` section: per-depth covered/total + untried/waived, close blockers (claimed only once a journal exists), evidence recommendations with `role`+`staffed` staffing state; `journal: false` honest degradation. **v1.19: U7×U8 predicted chains** — owning specialists staffed pre-hunt via `predicted_dispatches` (coverage gate still wins: park ⇒ recorded refusal), `predicted_chains` priority intel rides member dispatch payloads; CLI: `--plan/--run/--resume/--status --worker task-tool --timeout --no-recompose --preflight` |
...
| `tools/runtime/browser_driver.py` | Client-side validation behind a `BrowserDriver` protocol; no driver bound → `blocked-browser` evidence lead, never a fabricated result. Scope-gated. `validate_client_side`@~103, `blocked_browser_evidence`@~156. **v1.12: binding loader** — `load_default_driver()` (Playwright first, operator pin via `set_default_driver`), `driver_status()` audit fact |
| `tools/runtime/__init__.py` | 0 | package marker |
| `tools/runtime/browser_driver_playwright.py` | ~280 | **v1.12: real Playwright Chromium binding** (Phase 2.1) — navigate/console+pageerror capture/evaluate/screenshot evidence; own-worker-thread execution model (sync API marshalled off asyncio hosts — Claude Code bridge safe); evidence reset PER-NAVIGATION (stale console can never false-confirm the next lead); scope gate BEFORE browser start; lazy start; dialogs auto-dismissed; missing Playwright ⇒ honest availability fact, blocked-browser. `PlaywrightBrowserDriver`@~57 (`availability`@~75, `navigate`@~165, `screenshot`@~215) |
...
| `tools/instincts.py` | ~500 | **v1.24.0: the learning loop (INTEGRATION_PLAN Phase A; ECC continuous-learning-v2 pattern, MIT, attributed).** Mines EXISTING ledgers (lead journals via snapshot-dedupe, reporting refusals, U-regression failures, benchmark FP/FN, governor refusals) into `state/instincts/instincts.jsonl` (bugwolf-instinct/v1: id/kind/trigger/statement/action/evidence/confidence/occurrences/TTL). Active only at ≥2 occurrences; contradiction HALVES confidence; 90-day prune; idempotent distill (ledger is source of truth). Consumers weighting-only: `cockpit_section` (SessionStart top-5), `dispatch_modifier` (±0.25 bound), `order_techniques` (reorder-only demotion), `promote` (operator-gated global). `distill`@~390, `mine_techniques`@~120, `load_instincts`@~530 |
| `tools/head_to_head.py` | ~260 | **v1.24.0: the head-to-head harness (INTEGRATION_PLAN Phase C; ECC agent-eval methodology, MIT, attributed).** Corpus-derived tasks with per-task budget caps; deterministic judges (= benchmark `_signal_status` semantics, contender-blind); shipped hermetic pair (governed prober vs 8x spray baseline — same pass rate, 8x sends: the cost column exposes the spray); external runners recorded as skipped, never faked. Report → `state/benchmark/head_to_head.json`. `run_head_to_head`@~160, `hermetic_probe` (in benchmark.py) |
| `tools/understanding/canaries.py` | ~130 | **v1.24.0: injection canaries (INTEGRATION_PLAN Phase D; threat model from ECC the-security-guide).** Deterministic detectors over fetched page bodies (instruction-forgery, fake-system-prompt, agent-targeting, exfil-lure, hidden-instruction); attempts are FACTS into the U1 artifact with a bounded U8 confidence penalty (floor 0.05). Doctrine enforced by test: forged instructions never reach a dispatch payload. `scan_pages`@~80, `apply_confidence_penalty`@~130 |
| `tools/intel/` (base.py, channels.py) | ~420 | **v1.24.0: the intel lane (INTEGRATION_PLAN Phase E) — DEFAULT-OFF.** Agent-Reach Channel ABC ported (MIT, attributed): ordered backends, override that never hides working ones, real-probe check, credential-scrubbing doctor. Four credential-free channels (github_public/site_docs/rss_feed/jobs_page), direct (scope-gated replay) → jina (documented third party, `docs/INTEL_TRANSPARENCY.md`) failover; facts with provenance into U1/U2; dead channel = recorded fact; can never touch the scope/coverage gates. `doctor`@~150, `intel_digest`@~110 |
| `tools/runtime/understanding/antibot.py` | ~60 | **v1.24.0: antibot honesty (INTEGRATION_PLAN Phase F; AR `_is_antibot_page` heuristics, MIT, attributed + whole-body volume guard).** `_fetch_pages` excludes bot-wall challenge pages from U1 intake and records `{fact: "surface behind bot-wall", path}` facts instead of silently poisoning the business-lens model. `is_antibot_page`@~40 |
| `tools/u_regression.py` | ~330 | **v1.23.0: the corpus ⇄ Understanding-Layer regression bridge.** Each corpus case may declare `u_stages`; the bridge boots a mini-mission over the stub (per-credential crawl with an explicit fast-rate governor, business pages, OpenAPI), runs the U1–U9 pipeline, and verifies per case: declared stages produced artifacts, the coverage class HUNTS (browser/OAST classes exempt — they cannot be observed deterministically), a fact-checker per class confirms real model support (idor → object-ID inventory, mass-assignment → privilege-field family, business-logic → workflow paths), and negative controls assert ABSENCE (ID 999 ∉ inventory). A mismatch is a regression FAILURE. Report → `state/benchmark/u_regression.json`. `run_u_regression`@~237, `_run_mini_mission`@~190, `CLASS_TO_COVERAGE`@~80 |
| `tools/benchmark.py` | ~400 | Runs benchmark corpus (v2) against the operator-declared `--base-url`. **v1.22.1: H2.CL corpus items score live** — `transport: "h2cl"` cases boot the real H2 lab (stub backend + `H2Frontend` in the case's `h2cl_mode`: desync vs safe), the attacker leg is identical in both postures (C-L:0 + TE:chunked + smuggled request in the body), evidence is the VICTIM-observed body (smuggled marker present = anomaly), `request_smuggling` signals on that body; `enable_lab=False` (default) SKIPS lab cases with a recorded reason (hermetic CI stays hermetic; lab cases count toward neither TP nor FN — never a fake pass). `run_benchmark`@125, `_h2cl_transport`@~70, `_boot_h2_lab`@~45, `_signal_status`@~180. **v1.23.0: `enable_u_regression=True` runs the corpus's U-stage declarations through `tools/u_regression.py` (a live mini-mission over a booted stub); the verdict gains `u_regression_ok` — a vanished model fact fails the gate like a missed finding.** |
...
| `tools/harness_guard.py` | 304+ | Session contract verifier (`--verify --json`). `initialize`@144, `verify`@171. **v1.16: `--verify-install`** — offline fail-closed tree verification via `release_signing.verify_tree` (manifest hash + minisign signature) |
| `tools/release_signing.py` | ~430 | **v1.16 (Phase 6 opsec): signed releases + install verification.** `build_manifest` (deterministic SHA-256), `sign_manifest`/`sign_bytes` (Ed25519, minisign-style armor; absent key ⇒ honestly unsigned), `verify_manifest`/`verify_tree` (fail-closed: mismatch/missing/unlisted), `verify_bytes`, `check_update` (the opt-in home-beacon replacement: GitHub releases API, fail-silent). CLI `--build-manifest/--sign/--sign-file/--verify-file/--verify-tree/--check-update`. `build_manifest`@~66, `sign_bytes`@~130, `verify_manifest`@~180, `verify_tree`@~250, `check_update`@~300 |
...
| File | Lines | Purpose |
|---|---|---|
| `bridge/bugwolf-mcp.py` | 175 | MCP server (JSON-RPC 2.0 over stdio): `bugwolf_status/plan/run/leads/mode`; never crashes — failed calls return error objects. **v1.21: 14 tools** — added `bugwolf_capture_replay` (captures.jsonl → scope-filtered governed replay → drift summary). `dispatch`@~139, `main`@~158 |
...
| `commands/bugwolf-leads.md` | — | **v1.15 (Phase 5)**: lead ledger state machine UI — half-verdicts, deterministic next-mutation, kill guard (refused kill = park into chain pool), chain partners |
| `commands/bugwolf-scope.md` | — | **v1.15 (Phase 5)**: intake + live ScopeGate ALLOW/DENY preview (exclusion-beats-wildcard, lookalike refusal), harness contract + hook clear semantics |
| `commands/bugwolf-research.md` | — | **v1.15 (Phase 5)**: R1–R7 checkpoints (sequence + event-driven blocker/escalation), freshness gate, citation discipline |
| `commands/bugwolf-chain.md` | — | **v1.15 (Phase 5)**: deep_chain × differential paths × impact-focus ranking; chain leads carry the partner |
| `commands/bugwolf-doctor.md` | — | **v1.15 (Phase 5)**: 60-second smoke — lab runtimes + replay/browser engine suites vs the stub + packaging gates |
| `commands/bugwolf-understand.md` | — | **v1.15 (Phase 5): the front door** — Understanding Layer U1→U9 strict sequence (deterministic captures first), Assumption Ledger with dispro plans, coverage gate (park with reason), Hunting Brief output |
| `hooks/hooks.json` | 38 | SessionStart → preflight digest (cached); **PreToolUse → bugwolf_pretool_scope_hook.py (v1.14: scope enforcement on Bash + WebFetch)**; Stop → persistent-mode freeze |
| `hooks/bugwolf_stop_hook.py` | 84 | Thin stdlib shim: one JSON event in → one JSONL journal line (allowlisted scalar keys only) → one JSON decision out; a hook failure is logged and swallowed so it can never stall the harness |
| `hooks/bugwolf_pretool_scope_hook.py` | 251 | **v1.14: harness-level scope enforcement (Phase 3)** — PreToolUse hook intercepts Bash + WebFetch, extracts candidate hosts (curl/wget/nc/socat/ssh args, `--host`/`--connect-to`, scheme:// URLs, Host: header overrides), evaluates them against the mission's own scope contract (`state/scope_contract.json`); subdomain suffix rule, deny-beats-wildcard, lookalike-host refusal, loopback mirrors the engine gate. Deny = exit 2 (un-overridable) + `permissionDecision: "deny"` JSON, journaled to `scope_hook/denials.jsonl`. Fail-open on harness errors; inert without a mission contract; `clear` subcommand for operator override. `evaluate`@~60, `main`@~200 |
