...
Tool modules (BugWolf internals driven via Bash -- always through tools/runtime/sandbox.py): graphql_gid, domains.api.graphql_batch_analyzer, graphql_workflow
...
## Corpus upgrade v3 (Sept 2026): mutation input-type scope bypass

Distilled from the GraphQL scope-bypass ATO writeup (008) and the API
fuzzing corpus (016). Beyond introspection/node-ID work:

1. **Input-type census** [ACC-12]: introspect every *mutation input
   type*, not just the fields the UI sends. Admin-only optional params
   (`userId`, `orgId`) that the backend accepts without privilege
   checks are full-ATO: `updateUserProfile(input:{userId:"usr_victim",
   email:"attacker@x.com"})` → reset-email poisoning → takeover.
2. **Field-suggestion mining on disabled introspection** [API-09]:
   "Did you mean X?" errors reconstruct schemas (Clairvoyance-style
   wordlist rebuild) — the standard mitigation is not a mitigation.
3. **Body-first IDOR** [ACC-02]: proxy history is the surface — seven
   of the top-20 H1 IDOR reports were GraphQL variables, not URLs.
