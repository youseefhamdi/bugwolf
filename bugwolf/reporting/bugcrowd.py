...
# SCHEMA: bugwolf-reporting-bc-v1
...
def render(finding) -> str:
    ...
    vrt = _VRT_MAP.get(sev, "P5")
    lines = []
    lines.append(f"# {f.title or 'N/A'}")
    lines.append("")
    lines.append(f"**VRT Priority:** {vrt}  ")
    ...
    lines.append("## Impact")
    lines.append("")
    lines.append(f.description or "Refer to description.")
    lines.append("")
    lines.append("## Remediation Recommendation")
    lines.append("")
    lines.append("Apply input validation, output encoding, and least privilege. "
                 "Reference OWASP / CWE for this class.")
    lines.append("")
    if f.references:
