"""Moonshot Kimi backend (OpenAI-compatible)."""
...
class KimiBackend(HTTPClientMixin, BaseBackend):
    name = "kimi"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("kimi complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        ...
        log.info("kimi/moonshot does not host embeddings; returning dry-run")
        return self._dry_run_embed(inputs, model=m)
