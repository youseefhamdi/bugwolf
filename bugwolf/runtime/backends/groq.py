...
class GroqBackend(HTTPClientMixin, BaseBackend):
    name = "groq"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("groq complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        ...
        # Groq does not expose embeddings as of this writing; degrade to dry-run
        # rather than raising -- callers always need a deterministic answer.
        log.info("groq does not host embeddings; returning dry-run")
        return self._dry_run_embed(inputs, model=m)
