"""Cerebras backend (OpenAI-compatible)."""
...
class CerebrasBackend(HTTPClientMixin, BaseBackend):
    name = "cerebras"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("cerebras complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        ...
        log.info("cerebras does not host embeddings; returning dry-run")
        return self._dry_run_embed(inputs, model=m)
