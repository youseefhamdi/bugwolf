...
class MistralBackend(HTTPClientMixin, BaseBackend):
    name = "mistral"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("mistral complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        ...
        try:
            ...
            self._record(self.name, latency_ms=0.0, error=True)
            log.warning("mistral embed failed: %r", exc)
            raise
