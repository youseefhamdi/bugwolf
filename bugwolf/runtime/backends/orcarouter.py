"""OrcaRouter backend (OpenAI-compatible router)."""
...
class OrcaRouterBackend(HTTPClientMixin, BaseBackend):
    name = "orcarouter"
    api_key_env = "ORCAROUTER_API_KEY"
    base_url = "https://api.orcarouter.com/v1"
    default_model = "orca-router-default"
    quality_bar = "fast"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("orcarouter complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        ...
        log.info("orcarouter does not host embeddings; returning dry-run")
        return self._dry_run_embed(inputs, model=m)
