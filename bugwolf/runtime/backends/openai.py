...
class OpenAIBackend(HTTPClientMixin, BaseBackend):
    name = "openai"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("openai complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        ...
        try:
            ...
            self._record(self.name, latency_ms=0.0, error=True)
            log.warning("openai embed failed: %r", exc)
            raise
