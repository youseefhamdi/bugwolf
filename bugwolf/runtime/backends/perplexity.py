"""Perplexity backend (OpenAI-compatible)."""
...
class PerplexityBackend(HTTPClientMixin, BaseBackend):
    name = "perplexity"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("perplexity complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        ...
        log.info("perplexity does not host embeddings; returning dry-run")
        return self._dry_run_embed(inputs, model=m)
