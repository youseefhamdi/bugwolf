...
class TogetherBackend(HTTPClientMixin, BaseBackend):
    name = "together"
    ...
    def complete(self, prompt, *, model=None, temperature=None, max_tokens=None,
        ...
        try:
            ...
            log.warning("together complete failed: %r", exc)
            raise
    ...
    def embed(self, inputs, *, model=None):
        m = model or "togethercomputer/m2-bert-80M-8k-retrieval"
        if not self.available():
        ...
        try:
            ...
            self._record(self.name, latency_ms=0.0, error=True)
            log.warning("together embed failed: %r", exc)
            raise
