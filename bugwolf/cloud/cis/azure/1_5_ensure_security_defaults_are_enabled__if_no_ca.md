# CIS Azure Foundations 1.5 — Ensure Security Defaults are enabled (if no CA)
...
## Description
Ensure Security Defaults are enabled (if no CA). Baseline Security Defaults for tenants without CA.
