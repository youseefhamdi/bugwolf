# CIS AWS Foundations 1.0 — 3.1 Ensure no security groups allow ingress from 0.0.0.0/0 to remote server administration ports
...
## Description
Open RDP / SSH to the world is the canonical initial-access vector.
...
## Audit Procedure
1. `aws ec2 describe-security-groups | jq '.SecurityGroups[].IpPermissions[] | select(.FromPort == 22 or .FromPort == 3389)'`.
...
## Remediation
Restrict `0.0.0.0/0` to known corporate ranges; prefer SSM Session Manager.
