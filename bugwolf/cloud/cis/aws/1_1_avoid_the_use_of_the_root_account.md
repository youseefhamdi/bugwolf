# CIS AWS Foundations 1.0 — 1.1 Avoid the use of the root account
...
## Description
The root user has unrestricted access to all AWS resources. Daily operations should be performed using scoped IAM identities; the root account should be used only when absolutely necessary.
...
## Audit Procedure
1. Use `aws iam get-account-summary` and inspect the `AccountMFAEnabled` flag.
2. Check `aws iam get-credential-report` for the root user's last activity.
3. Confirm that no IAM policy allows `Principal: *` with `Effect: Allow` rooted at root.
...
## Remediation
Remove or rotate root access keys, enable MFA, and create a scoped admin IAM user for daily work.
