# CIS AWS Foundations 1.0 — 1.9 Ensure MFA is enabled for IAM users with console access
...
## Description
Console users without MFA are a high-value phishing target.
...
## Audit Procedure
1. Filter credential report for `password_enabled == true && mfa_active == false`.
...
## Remediation
Force MFA enrollment at next login via `aws iam update-login-profile`.
