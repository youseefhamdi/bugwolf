# CIS AWS Foundations 1.0 — 1.2 Ensure MFA is enabled for the root account
...
## Description
MFA adds a second factor that defeats credential theft. Without it the root account is one leaked password away from full account takeover.
...
## Audit Procedure
1. Run `aws iam get-account-summary | jq .AccountMFAEnabled`.
2. If false, escalate immediately.
...
## Remediation
Attach a hardware MFA device (YubiKey / FIDO2 preferred) to the root user.  Avoid virtual MFA where possible.
