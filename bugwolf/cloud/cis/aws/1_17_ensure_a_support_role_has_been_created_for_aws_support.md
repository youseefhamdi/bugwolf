# CIS AWS Foundations 1.0 — 1.17 Ensure a support role has been created for AWS Support
...
## Description
Granting `Support:*` to root is acceptable only if scoped to a role.
...
## Audit Procedure
1. `aws iam list-entities-for-policy --policy-arn arn:aws:iam::aws:policy/AWSSupportAccess`.
...
## Remediation
Create a dedicated `SupportUser` role; do not attach to root.
