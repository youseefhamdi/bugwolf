#!/usr/bin/env python3
"""Persistent no-skip workflow controller for BugWolf.
...
try:
    ...
    from tools.safety import (
        AuthorizationError, load_authorized_scope, safe_path, target_in_scope,
    )
...
STAGE_DESCRIPTIONS = {
    ...
    "passive-recon": "collect only authorized passive target intelligence",
    "asset-intelligence": "normalize, scope-filter, and diff the discovered assets",
    ...
    "validation": "perform only explicitly authorized, bounded validation",
...
def _relative_or_absolute(project: Path, value: str) -> Path:
    """Resolve an artifact path without allowing project-root escape."""
    try:
...
class WorkflowController:
    ...
    def _validate_stage_artifacts(self, name: str, artifacts: Iterable[str]) -> List[str]:
        ...
        if name in {"setup", "environment-preflight", "passive-recon",
                    "asset-intelligence", "technology-fingerprint", "maps",
                    "research", "coverage-plan"} and not paths:
    ...
    def _validate_authorization(self, scope_file: Optional[str]) -> str:
        ...
        try:
            scope = load_authorized_scope(_relative_or_absolute(self.project, chosen))
            if not target_in_scope(self.target, scope):
                raise AuthorizationError(f"target is not in scope: {self.target}")
    ...
    def _validate_research(self) -> str:
        ...
        required = ["pre-hunt", "post-recon", "post-maps", "bypass",
...
def main() -> int:
    ...
    action.add_argument("--start", action="store_true", help="initialize the target workflow")
    ...
    try:
        ...
        if args.start:
            result = controller.initialize(force=False)
