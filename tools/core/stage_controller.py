...
# P0 deep-hunt artifacts recognized by the workflow (research / coverage-plan).
# These are produced by the modular domain tools (http_smuggling_detector,
# parser_differential, jwt_forgery) and, when present, participate in the
# stage artifact contract.
P0_RESEARCH_ARTIFACTS = (
    "bypass/waf-payloads",      # research/<target>/bypass/waf-payloads-<stack>.json
    "auth/jwt-forgery-plans",   # research/<target>/auth/jwt-forgery-plans.json
)
P0_COVERAGE_ARTIFACTS = (
    "discovery/smuggling-plan",  # recon/<target>/discovery/smuggling-plan.jsonl
)


def _mandatory_ordered_subsequence(sequence, required) -> bool:
    """True when ``required`` appears in order inside ``sequence``.

    Mirrors research_loop.mandatory_ordered_subsequence so both enforcement
    points agree: hierarchical sub-checkpoints may sit between the mandatory
    7 without invalidating the sequence.
    """
...
class WorkflowController:
    ...
    def status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supplementary_artifacts": [
                ...
                for path in self._supplementary_artifacts(current)
            ],
    ...
    def refresh_artifact_hashes(self, stage_name: str) -> Dict[str, Any]:
        ...
        per-asset recon appended to asset-intel).
        ...
        Integrity is verified for every completed stage EXCEPT the one being
        refreshed — that stage's artifacts are legitimately in flux (the whole
        point of the refresh), so validating them first would make the
        mechanism unusable.  The audit note + new chain snapshot still make the
        change visible to any later verification.
        """
        data = self._load()
        self._validate_completed_integrity(data, skip=stage_name)
    ...
    def _validate_completed_integrity(self, data: Dict[str, Any],
                                      skip: Optional[str] = None) -> None:
        """Refuse to advance after a completed artifact was modified.

        ``skip`` names a stage whose artifact changes are tolerated — used by
        refresh_artifact_hashes, where the refreshed stage is legitimately in
        flux and the change is separately audited.
        """
        for stage in data.get("stages", []):
            if stage.get("status") not in {"complete", "complete_pending"}:
                continue
            if skip and stage.get("name") == skip:
                continue
    ...
    def _supplementary_artifacts(self, name: str) -> List[Path]:
        """P0 deep-hunt artifacts recognized by the stage, when present.

        Supplementary artifacts are merged into the stage's recorded + hashed
        artifact list (and surfaced in status) but are never *required* — a
        campaign without a WAF/GraphQL surface must still advance.  Supplying
        them explicitly via ``--artifact`` also validates them.
        """
        root = self.project
        target = self.target
        research_dir = root / "research" / target
        recon_dir = root / "recon" / target
        if name == "research":
            out: List[Path] = []
            payloads = sorted(research_dir.glob("bypass/waf-payloads-*.json"))
            out.extend(payloads)
            for auth_artifact in ("jwt-forgery-plans.json",
                                  "oauth-flow-plans.json"):
                candidate = research_dir / "auth" / auth_artifact
                if _nonempty(candidate):
                    out.append(candidate)
            for llm_artifact in ("llm/agentic-tool-auth-plans.json",
                                 "llm/rag-poisoning-plans.json"):
                candidate = research_dir / llm_artifact
                if _nonempty(candidate):
                    out.append(candidate)
            for deep_artifact in ("contracts/triage-verdicts.json",
                                  "contracts/price-manipulation-plans.json",
                                  "verification/lab-plans.json",
                                  "advisor/seed-proposals.json",
                                  "learning/failure-bypass-candidates.json",
                                  "chains/graph-ai-proposals.json"):
                candidate = research_dir / deep_artifact
                if _nonempty(candidate):
                    out.append(candidate)
            return out
        if name in ("passive-recon", "asset-intelligence"):
            out = []
            for artifact in ("asset-intel/history.jsonl",
                             "asset-intel/delta.json"):
                candidate = recon_dir / artifact
                if _nonempty(candidate):
                    out.append(candidate)
            return out
        if name == "coverage-plan":
            out = []
            for artifact in ("smuggling-plan.jsonl", "graphql-plans.json",
                             "bopla-matrix.json", "deep-link-plans.json",
                             "mobile-policy-check.json",
                             "ato-chain-plans.json"):
                candidate = recon_dir / "discovery" / artifact
                if _nonempty(candidate):
                    out.append(candidate)
            # Cloud capability registry lives under state/capability/.
            capability = root / "state" / "capability" \
    ...
    def _validate_stage_artifacts(self, name: str, artifacts: Iterable[str]) -> List[str]:
        ...
        if missing:
            raise WorkflowError(
                ...
                + ", ".join(missing))
        # Supplementary P0 artifacts (smuggling-plan, waf-payloads,
        # jwt-forgery-plans) are recorded + hashed with the stage when present.
        supplementary = [
            path for path in self._supplementary_artifacts(name)
            if _nonempty(path) and path not in paths
        ]
        paths = list(paths) + supplementary
    ...
    def _validate_research(self) -> str:
        ...
        # Hierarchical sub-checkpoints may be interleaved between the mandatory
        # 7 (they add depth, never replace) — verify the mandatory sequence
        # appears in order, exactly like research_loop.verify_sequence does.
        if not _mandatory_ordered_subsequence(sequence, required):
            raise WorkflowError(
