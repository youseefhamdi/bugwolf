...
> Generated on 2026-08-26T20:48:04+00:00 from `main@06b08ff` by `scripts/generate_audit.py`. All counts are computed from the live tree; do not edit them by hand.

## 1. Scale

- Python modules under `tools/`: **111** (55088 lines) + **12** `__init__.py` package markers.
- CLI-capable modules (argparse/`__main__`): **95**
- Group sizes: core **8**, domains **14**, intelligence **3**, recon **1**, validation **2**

## 2. Test suite

- Test files: **70**
- Discovered tests: **915**
...
| `tools/core/campaign_orchestrator.py` | 2089 | BugWolf Campaign Orchestrator — APT Commander (Stage 2 rebuild). |
...
| `tools/core/live_executor.py` | 801 | BugWolf Live Execution Harness — real probes, real evidence. |
...
| `tools/crypto_vault.py` | 539 | BugWolf Crypto Vault — Encrypted artifact store with secure deletion. |
| `tools/wordlist_gen.py` | 538 | BugWolf Custom Wordlist Generator v1.0.0 |
